...
for j = 1:6
  ...
  for i = 1:columns(parts)
    ...
    bayes_indep(i,j) = mean(bayescls(train_part(:,2:end), @pdf_indep, pdfindep_para, apriori) != train_part(:,1));
    bayes_multi(i,j) = mean(bayescls(train_part(:,2:end), @pdf_multi, pdfmulti_para, apriori) != train_part(:,1));
    %bayes_parzen(i,j) = mean(bayescls(train_part(:,2:end), @pdf_parzen, pdfparzen_para, apriori) != train_part(:,1));
  end
end
...
display(bayes_indep_parts)
display(bayes_multi_parts)
